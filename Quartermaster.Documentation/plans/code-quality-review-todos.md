...
- [x] **Login tokens never expire.** Fixed: `TokenRepository.LoginUser` now sets `Expires = UtcNow + auth.token.lifetime_days` (new global option, default 7) on issue, and `ValidateLoginToken` slides expiry forward by the same window on each successful validate (sliding-window, ExtendType.Usage). Expiration policy moved out of the static `TokenExtensions` entity layer into `TokenRepository` (resolves the in-code TODO). The dead static `CheckLoginToken`/`CheckSimpleToken` helpers + their `UpdateTokenExpiration` are left untouched; they'll be removed with the separate "TokenSecurityScope dead-code" cleanup. Added `Issued_token_has_expiry_populated_within_configured_lifetime` + `Successful_validation_slides_expiry_forward` tests; full suite green except pre-existing Playwright E2E (browsers not installed in this env).
- [ ] **`TokenSecurityScope` (None / IP / BrowserFingerprint) is dead.** `Quartermaster.Data/Tokens/TokenRepository.cs:33-51` (`ValidateLoginToken`) hashes `$"{tokenContent};"` — fingerprint hardcoded to `""`, `SecurityScope` never read, `UpdateTokenExpiration` never called. Decision needed: implement or delete the enum so it stops misrepresenting the security posture.
- [ ] **`X-Forwarded-For` trusted unconditionally for lockout keying.** `Quartermaster.Server/Users/LoginEndpoint.cs:99-103` (`GetClientIp`). Combined with `(ipAddress, identifier)`-keyed lockout, an attacker spoofs the header to bypass per-IP lockout entirely. Use `KnownProxies`/`KnownNetworks` on `ForwardedHeadersOptions` or only trust the header on configured proxies.
- [ ] **Delete `EndpointProcessors/ChapterPermissionRequirement.cs` and `EndpointProcessors/ChapterRequirement.cs`.** `ChapterPermissionRequirement:31-34` reads `UserId` from a request header — trivial impersonation if ever wired up. `ChapterRequirement:13` checks a `"Chapter"` claim the auth handler never sets. Both files are currently unreferenced; delete before someone uses them.

### Authorization & IDOR

- [ ] **Permission inheritance is silently disabled across the system.** `Quartermaster.Data/UserChapterPermissions/UserChapterPermissionRepository.cs:146` — `IsViewPermission` checks `EndsWith("_view")` but real identifiers are PascalCase (`ViewUsers`, `ViewMotions`, …). Effectively no view permission is ever inherited from a parent chapter. Fix the check or rename the identifiers; then audit whether any tests/permission grants relied on the (broken) behavior.
- [ ] **`ChapterOfficerAddEndpoint` doesn't verify member↔chapter.** `Quartermaster.Server/ChapterAssociates/ChapterOfficerAddEndpoint.cs:36-57` — an `EditOfficers` holder in chapter A can promote a member from chapter B by passing their MemberId; `GrantDefaultPermissions(member.UserId.Value, req.ChapterId)` then gives that user permissions in the caller's chapter. Verify `req.MemberId`'s chapter matches `req.ChapterId` before granting.
- [ ] **`MembershipApplicationRepository.List` widens on empty `chapterIds`.** `Quartermaster.Data/MembershipApplications/MembershipApplicationRepository.cs:31-49` — `Count > 0` gate means empty list returns *all* applications. Confirm callers' intent (likely "no chapters → no rows"), then fix the gate.
...
- [ ] **SAML signature has no replay / audience / NotBefore-NotOnOrAfter checks.** `Quartermaster.Server/Users/SamlLoginConsumeEndpoint.cs:60-70` calls bare `Saml.Response(cert, req.SamlData).IsValid()`. Older AspNetSaml versions have known signature-validation gaps. Track `AssertionID` for the assertion lifetime, enforce `NotBefore`/`NotOnOrAfter`, validate `Audience` and `Destination`.
...
- [ ] **Scrub emails from auth-flow logs.** `Quartermaster.Server/Users/SamlLoginConsumeEndpoint.cs:90` and `OidcCallbackEndpoint.cs:122` log full email at info level.
...
- [ ] **Composite-permission tables have no PK / no unique constraint.** `Quartermaster.Data/Migrations/M001_InitialStructureMigration.cs:105-132` — `UserGlobalPermissions`, `UserChapterPermissions`, `RolePermissions`, `ChapterOfficers`. Duplicates blocked only by an `AddForUser` pre-check (race). Add composite PKs / unique indexes — fold into M001 while pre-prod.
- [ ] **Multi-write operations lack transactions.** `Quartermaster.Data/Motions/MotionRepository.cs:112-173` (`TryAutoResolve`) does 5+ writes with no `BeginTransaction()`. Same pattern in `RoleRepository.Delete`/`SetPermissions`, `OptionRepository.SetValue`, every `Update(...)` that pairs Set+LogFieldChange. Wrap in `_db.BeginTransaction()`.
