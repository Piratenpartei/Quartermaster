...
- [x] **Meeting system with agenda** — full implementation shipped. `Meeting` entity (ChapterId, Title, MeetingDate, Status, Visibility, auto-PDF-on-archive) + `AgendaItem` entity (self-referencing ParentId for subitems, SortOrder, ItemType, optional MotionId FK, Notes, Resolution, StartedAt/CompletedAt). Status lifecycle Draft→Scheduled→InProgress→Completed→Archived with auto-resolve of linked motions on Completed and PDF snapshot on Archived. Hierarchical numbering (1 / 1.1 / 1.2) computed client-side. Motion votes cast during a meeting carry `MeetingId` FK; `TryAutoResolve` disabled for meeting-linked votes (manual resolve still works). Public/Private visibility; Private requires direct chapter_officer or general_chapter_delegate role (no inheritance). New `general_chapter_delegate` system role + `Role.InheritsToChildren` flag for the non-inheriting delegate pattern. QuestPDF-based protocol export (md/html/pdf formats, frozen snapshot at archive time). Full Blazor UI: list/create/detail pages with three modes (Draft/Scheduled editor with agenda tree + reorder + motion picker, InProgress live minute-taking with auto-save notes + vote UI + close-vote, Completed/Archived read-only + PDF download). 136 new tests total.
