...
- [ ] **Auto-grant chapter permissions for officers** — when a chapter officer is added, automatically grant their linked user the standard chapter-scoped permissions (ViewMotions, VoteMotions, ViewEvents, ViewMembers, ViewOfficers, etc.) for that chapter; when an officer is removed, revoke those auto-granted permissions. Admins can still manually adjust permissions beyond the defaults.
