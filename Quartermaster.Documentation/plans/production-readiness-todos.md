...
- [ ] **Home page dashboard** — show permission-gated summary widgets on the landing page: open membership applications, pending due selections needing approval, active motions, upcoming events, import service status, recent audit log entries. Each widget visible only to users with relevant view permissions, with deep links to the detail pages.
